...
const RoleSelectionModal = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-fade-in overflow-y-auto">
      <div className="relative w-full max-w-5xl bg-white/85 backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 md:p-12 shadow-2xl border border-white/40 flex flex-col items-center animate-scale-up max-h-[96vh] sm:max-h-[90vh] overflow-y-auto">
        
        {/* Subtle Liquid Gradient Blobs */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-2xl sm:rounded-[2.5rem] -z-10">
        ...
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 sm:top-6 sm:right-6 text-slate-400 hover:text-slate-600 transition-colors p-2 rounded-full hover:bg-slate-50 cursor-pointer z-10"
        ...
        {/* Modal Header */}
        <div className="text-center max-w-2xl mb-6 sm:mb-8 md:mb-10 mt-2 sm:mt-0">
          <h2 className="font-poppins text-xl sm:text-2xl md:text-4xl font-normal text-slate-800 tracking-tight leading-tight">
            Welcome to <span className="text-[#DC2626] font-medium">BnC</span> LEG
          </h2>
          <p className="font-geist text-xs sm:text-sm md:text-lg text-slate-500 mt-2 sm:mt-4 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-10 w-full">
          ...
          <div className="border border-white/30 bg-white/20 backdrop-blur-xl rounded-xl sm:rounded-[2.5rem] p-5 sm:p-6 md:p-10 flex flex-col items-center text-center shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden">
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-white/40 border border-white/50 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6">
              <FaBriefcase className="w-5 h-5 sm:w-6 sm:h-6 text-slate-500" />
            </div>
 
            <h3 className="font-poppins text-lg sm:text-xl md:text-2xl font-bold text-slate-900 mb-2 sm:mb-4">
              Claim a Project Mandate
            </h3>
            
            <p className="font-geist text-xs sm:text-sm md:text-base text-slate-500 leading-relaxed mb-4 sm:mb-6 md:mb-8 flex-grow">
              I represent a consulting firm or agency and I am looking for verified corporate mandates to work on.
            </p>
 
            <div className="w-full space-y-2 sm:space-y-3 mt-auto">
              <button
                onClick={() => handleSignUp('provider')}
                className="w-full bg-[#0f294a] hover:bg-slate-900 text-white py-2.5 sm:py-3.5 md:py-4 rounded-lg sm:rounded-xl font-poppins font-semibold text-xs sm:text-sm transition-all cursor-pointer flex items-center justify-center gap-1 shadow-sm hover:shadow-md"
              >
                Sign Up &rarr;
              </button>
              ...
              <button
                ...
                className="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 py-2.5 sm:py-3.5 md:py-4 rounded-lg sm:rounded-xl font-poppins font-semibold text-xs sm:text-sm transition-all cursor-pointer"
              >
                Log In
              </button>
            </div>
          ...
          <div className="border border-white/30 bg-white/20 backdrop-blur-xl rounded-xl sm:rounded-[2.5rem] p-5 sm:p-6 md:p-10 flex flex-col items-center text-center shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden">
            ...
            {/* Minimal Icon Wrapper */}
            <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-white/40 border border-white/50 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6">
              <FaUserTie className="w-5 h-5 sm:w-6 sm:h-6 text-slate-500" />
            </div>
