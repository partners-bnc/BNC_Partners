...
import { fetchPartnerData, getSessionUser, logout, submitPartnerAgreement } from '../../lib/supabaseData';
...
const DEFAULT_PROVIDER_TRANSACTIONS = [
  { id: 'TX-89102', date: 'Oct 24, 2026', client: 'Acme Corp', service: 'Tax Strategy Consultation', amount: 1250, status: 'Completed' },
  { id: 'TX-89103', date: 'Oct 21, 2026', client: 'Jane Doe', service: 'Portfolio Review', amount: 300, status: 'Completed' },
  { id: 'TX-89104', date: 'Oct 18, 2026', client: 'Global Tech', service: 'Estate Planning', amount: 500, status: 'Completed' }
];

const DEFAULT_PROVIDER_CONSULTATIONS = [
  { id: 'c-1', date: 'Oct 26, 2026', client: 'Sarah Jenkins', service: 'Portfolio Review', time: '10:00 AM', link: 'https://meet.google.com/abc-defg-hij' },
  { id: 'c-2', date: 'Oct 28, 2026', client: 'Tech Solutions Inc.', service: 'Tax Strategy Consultation', time: '02:00 PM', link: 'https://meet.google.com/klm-nopq-rst' },
  { id: 'c-3', date: 'Nov 02, 2026', client: 'Robert Chen', service: 'Estate Planning', time: '11:30 AM', link: 'https://meet.google.com/uvw-xyz-123' }
];

const DEFAULT_CLIENT_BOOKINGS = [
  { id: 'cb-1', date: 'Oct 26, 2026', expert: 'Tavisha Sharma', service: 'Portfolio Review', time: '10:00 AM', status: 'Scheduled' },
  { id: 'cb-2', date: 'Nov 05, 2026', expert: 'Rajesh Kumar', service: 'Virtual CFO Services', time: '03:00 PM', status: 'Scheduled' }
];

const DEFAULT_CLIENT_INVOICES = [
  { id: 'INV-10294', date: 'Oct 12, 2026', description: 'Initial Tax consultation', amount: 250, status: 'Paid' },
  { id: 'INV-10298', date: 'Oct 16, 2026', description: 'Growth Capital Consultation', amount: 150, status: 'Unpaid' }
];
...
const ALL_DIRECTORY_SERVICES = [
  {
    ...
    category: 'Finance',
    bullets: ['Financial Planning & Analysis (FP&A)', 'Budget & Cost Management', 'Reporting & MIS', 'Cash Flow Management'],
    price: 350,
    unit: 'session',
  ...
  },
  {
    ...
    category: 'Finance',
    bullets: ['Corporate Tax filings', 'Audit Support & Review', 'Regulatory reporting compliance', 'Tax Advisory & Advisory'],
    price: 250,
    unit: 'hr',
  ...
  },
  {
    ...
    category: 'Growth',
    bullets: ['LAP OD Limit (Loan Against Property)', 'Unsecured Business Loans', 'Overdraft (OD) Limits', 'Project finance advisory'],
    price: 450,
    unit: 'retainer',
  ...
  },
  {
    ...
    category: 'Security',
    bullets: ['Global Data Privacy (GDPR) Compliance', 'SOC Certification (Type 1 & 2)', 'HIPAA Compliance & Mitigation', 'Vulnerability scanning'],
    price: 550,
    unit: 'audit',
...
const UserDashboard = () => {
  ...
  const [providerCredentials, setProviderCredentials] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : {
      legalName: '',
      primaryDomain: 'Wealth & Financial Planning',
      experience: 5,
      firmName: ''
    };
  });
  ...
  const handleVoiceRequirementSubmit = async (payload) => {
    ...
    setTimeout(() => {
      ...
      const isSecurity = requirementText.toLowerCase().includes('cyber') || requirementText.toLowerCase().includes('gdpr') || requirementText.toLowerCase().includes('hack');
      ...
      const newBooking = {
        id: `cb-${Date.now()}`,
        date: 'Oct 30, 2026',
        expert: matchedExpert.name,
        service: isSecurity ? 'AI Audit Consultation' : 'Financial Portfolio Consultation',
        time: '11:00 AM',
        status: 'Scheduled'
      };
  ...
  const handleBookServiceSubmit = (e) => {
    ...
    const newBooking = {
      ...
      expert: selectedServiceToBook?.expert || 'Assigned Consultant',
      service: selectedServiceToBook?.title || 'Consultation',
  ...
  const filteredDirectory = useMemo(() => {
    return ALL_DIRECTORY_SERVICES.filter(service => {
      const matchSearch = service.title.toLowerCase().includes(exploreSearch.toLowerCase()) ||
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-650 dark:text-slate-400 font-medium">Initializing Dashboard Components...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Header
        ...
        activeTab={activeTab}
      />
      <div className="min-h-screen bg-[#FAFCFF] dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col relative transition-colors duration-300 font-['Plus_Jakarta_Sans',sans-serif] overflow-hidden">
        ...
        <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full space-y-8 relative z-10">
          ...
          <div className="flex flex-col items-center justify-center text-center relative pt-4">
            {/* 3D Curved Box Badge */}
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 shadow-[inset_0_-2px_4px_rgba(0,0,0,0.05),0_4px_12px_rgba(0,0,0,0.05),0_1px_3px_rgba(0,0,0,0.03)] mb-4 transition-all">
              <span className="w-2.5 h-2.5 rounded-full bg-[#E52E38] animate-pulse"></span>
              <span className="text-[11px] font-bold tracking-widest text-[#0F2A4A] dark:text-slate-200 uppercase font-sans">
                {currentRole === 'provider' ? 'Service Provider' : 'Service Consumer'}
              </span>
            </div>
            
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold text-[#0F2A4A] dark:text-white tracking-tight">
              Welcome back, Anshu
            </h1>
          </div>
      ...
      {isCredentialsModalOpen && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl relative">
            <button onClick={() => setIsCredentialsModalOpen(false)} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
              <X className="w-5 h-5" />
            </button>

            <span className="text-[10px] font-bold tracking-widest text-[#E52E38] uppercase">PROVIDER SETUP • CREDENTIALS</span>
            <h3 className="text-xl font-black text-slate-900 dark:text-white mt-1 mb-2">Complete Profile Credentials</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 font-semibold">Enter your credentials to match with enterprise projects.</p>
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  name="legalName"
                  defaultValue={providerCredentials.legalName || `${partnerData?.firstName} ${partnerData?.lastName}`}
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Primary Domain</label>
                  <select name="primaryDomain" defaultValue={providerCredentials.primaryDomain} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Organization / Firm</label>
                <input
                  type="text"
                  name="firmName"
      ...
      {isBankModalOpen && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl relative">
            <button onClick={() => setIsBankModalOpen(false)} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
              <X className="w-5 h-5" />
            </button>

            <span className="text-[10px] font-bold tracking-widest text-[#E52E38] uppercase">PROVIDER SETUP • FINANCES</span>
            <h3 className="text-xl font-black text-slate-900 dark:text-white mt-1 mb-2">Link Bank Account</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 font-semibold">Enter your bank details to secure consulting payouts safely.</p>
            ...
            <form onSubmit={handleBankSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Routing Transit Number</label>
                  <input
                    type="text"
                    name="routingNo"
