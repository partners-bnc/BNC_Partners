    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Instantly Map Your Mandate</h2>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Type Your Requirements</label>
            <textarea
              ...
              className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs focus:ring-2 focus:ring-[#E52E38]/20 focus:outline-none"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <button
              ...
              className="w-full sm:w-auto px-5 py-3.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 text-slate-700 dark:text-slate-200 text-xs font-extrabold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        {matchedExpertResult && (
          <div className="pt-6 border-t border-slate-100 dark:border-slate-800 space-y-4 animate-fade-in">
            <div className="p-4 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 flex items-start gap-4">
              <CheckCircle className="w-8 h-8 text-emerald-500 shrink-0" />
              <div>
                <h3 className="text-sm font-bold text-emerald-800 dark:text-emerald-400">Match Found & Consultation Confirmed!</h3>
            ...
            <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">{matchedExpertResult.name}</h4>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">{matchedExpertResult.firm} • Rating {matchedExpertResult.rating}★</p>
                </div>
              </div>
            </div>
          </div>
