...
const ConsumerInvoices = ({ clientInvoices, handlePayInvoice }) => {
  return (
    <div className="bg-white p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-sm space-y-6 animate-fade-in">
      <div>
        <h2 className="text-xl font-black text-slate-900 tracking-tight">Payments & Invoices</h2>
        <p className="text-xs text-slate-500 font-medium">Keep track of payments, audit retainers, and consultations billings.</p>
      </div>
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-700">
            {clientInvoices.map(inv => (
              <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="py-4 px-2 font-bold text-slate-900">{inv.description}</td>
                <td className="py-4 px-2 text-right font-black text-slate-900">${inv.amount.toLocaleString()}.00</td>
