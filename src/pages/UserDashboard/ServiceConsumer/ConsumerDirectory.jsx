      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80 font-medium">
          <input
            ...
            className="w-full px-4 py-2.5 pl-10 rounded-xl border border-slate-200 bg-slate-50 text-xs focus:ring-2 focus:ring-[#E52E38]/20 focus:outline-none"
          />
      ...
      {isBookModalOpen && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md">
          <div className="w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-2xl relative">
            <button onClick={() => { setIsBookModalOpen(false); setSelectedServiceToBook(null); }} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 text-slate-400">
              <X className="w-5 h-5" />
            </button>

            <span className="text-[10px] font-bold tracking-widest text-[#E52E38] uppercase">BOOK APPOINTMENT</span>
            <h3 className="text-xl font-black text-slate-900 mt-1 mb-2">Book Consultation Call</h3>
            <p className="text-xs text-slate-500 mb-6 font-semibold">Schedule a strategy meeting for service: <strong>{selectedServiceToBook?.title}</strong></p>
            ...
            <form onSubmit={handleBookServiceSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    name="date"
                    required
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-xs text-slate-600"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Select Time</label>
                  <select name="time" className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-xs">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Assigned Partner / Agency</label>
                <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs font-bold">
                  BNC Advisory Network Consultant (${selectedServiceToBook?.price} / {selectedServiceToBook?.unit})
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Add Specific Goals / Notes</label>
                <textarea
                  name="notes"
