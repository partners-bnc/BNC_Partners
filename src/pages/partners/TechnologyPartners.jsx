...
import { FaArrowRight, FaPlug, FaDatabase, FaRobot, FaCogs } from 'react-icons/fa';
...
const TechnologyPartners = () => {
  ...
  const focusCardsText = t('partnerPages.technology.cards', { returnObjects: true });
  ...
  return (
    <div className="bg-white">
      <section className="relative w-full bg-[#000000] text-white -mt-16 pt-28 pb-24 min-h-[320px] overflow-hidden rounded-b-[48px]">
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 pl-10 md:pl-16 relative z-10">
          <div className="max-w-3xl">
            <p className="text-xs uppercase tracking-[0.25em] text-white/70">{t('partnerPages.technology.label')}</p>
            <h1 className="mt-3 text-3xl md:text-4xl font-semibold text-white">
              {t('partnerPages.technology.title')}
            </h1>
            <p className="mt-3 text-white/80">
              {t('partnerPages.technology.subtitle')}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 md:px-6 lg:px-8 py-12 pl-10 md:pl-16">
        <div className="grid lg:grid-cols-[1.55fr_0.75fr] gap-8">
          <div className="space-y-6">
            <div>
              <h2 className="text-[#DC2626]xl font-semibold text-slate-900">{t('partnerPages.shared.sectionTitle')}</h2>
              <p className="mt-2 text-slate-600">
                {t('partnerPages.technology.sectionSubtitle')}
              </p>
            </div>
              ...
              {focusCards.map((card) => (
                ...
                >
                  <div className="relative h-32 overflow-hidden">
                    ...
                    />
                    <div className="absolute top-3 left-3 rounded-lg bg-white/95 backdrop-blur-sm p-2 text-[#DC2626] shadow-sm">
                      <card.icon className="text-[18px]" />
                    </div>
                  </div>
                  <div className="flex flex-col flex-grow p-4">
                    <h3 className="font-semibold text-slate-900 group-hover:text-[#DC2626] transition-colors">
                      {card.title}
                    </h3>
          ...
          <div className="rounded-3xl border border-slate-100 bg-gradient-to-b from-[#f8fbff] to-white p-7 shadow-[0_12px_40px_rgba(32,70,129,0.08)] max-w-md w-full lg:justify-self-end h-fit">
            <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-4">
              ...
              <span className="rounded-full bg-red-50 px-3 py-1 text-xs uppercase tracking-wider text-[#DC2626] font-semibold">
                {sidebarContent.stepsLabel}
              </span>
            </div>
            
            <div className={`mt-6 flex flex-col relative before:absolute before:inset-y-0 before:w-[2px] before:bg-gradient-to-b before:from-[#DC2626]/20 before:to-transparent ${timelineLinePosition}`}>
              {sidebarContent.steps.map((step, index) => (
                <div key={step} className="relative flex items-start gap-4 mb-5 last:mb-0 group">
                  <div className="relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-4 border-white bg-[#000000] text-white text-sm font-bold shadow-sm transition-transform duration-300 group-hover:scale-110">
                    {index + 1}
                  </div>
                  <div className="pt-1.5 pb-1">
                    <p className="text-[10px] uppercase tracking-wider text-[#DC2626]/70 font-bold mb-0.5">
            ...
            <Link
              to="/?open=partner"
              className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#000000] to-[#DC2626] text-white px-5 py-3.5 text-sm font-semibold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
            >
              {sidebarContent.cta}
